...
/**
 * Modul pro na��t�n� dat z EDF souboru.
 * 
...
public class EdfFormatLoader implements DataFormatLoader {
	...
	private void loadGlobalHeader() throws EOFException, CorruptedFileException {
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 80)) == 80) {
			...
			throw new CorruptedFileException(
					"Can't read local patient identification (EOF).");
		}
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 80)) == 80) {
			...
			throw new CorruptedFileException(
					"Can't read local recording identification (EOF).");
		}
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 8)) == 8) {
			...
			throw new CorruptedFileException(
					"Can't read startdate of recording (EOF).");
		}
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 8)) == 8) {
			...
			throw new CorruptedFileException(
					"Can't read starttime of recording (EOF).");
		}
		timeOfAcquisition = new String(buffer, 0, 8).trim();

		header.setDateOfAcquisition(transformTimeAndDayToCalendar(dayOfAcquisition,
				timeOfAcquisition));
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 8)) == 8) {
			...
			throw new CorruptedFileException(
					"Can't read number of bytes in header record (EOF).");
		}
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 8)) == 8) {
			...
			throw new CorruptedFileException(
					"Can't read number of data records (EOF).");
		}
		numberOfDataRecords = Integer.valueOf(new String(buffer, 0, 8).trim())
				.intValue();
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 8)) == 8) {
			...
			throw new CorruptedFileException(
					"Can't read duration of data record (EOF).");
		}
		durationOfDRecord = (Integer.valueOf(new String(buffer, 0, 8).trim())
				.intValue());
		...
		if ((curBytesRead = nioStream.read(buffer, 0, 4)) == 4) {
			...
			throw new CorruptedFileException(
					"Can't read number of signals in data record (EOF).");
		}
		numberOfSignals = (Integer.valueOf(new String(buffer, 0, 4).trim())
				.intValue());
	...
	private void loadChannelsHeader() throws CorruptedFileException {
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException("Can't read label (EOF). Channel: "
						+ i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read transducer type (EOF). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read physical dimension (EOF). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read physical minimum (EOF). Channel: " + i, e);
			} catch (NumberFormatException e) {
				throw new CorruptedFileException("Can't read physical minimum ("
						+ e.getMessage() + " not a number). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read physical maximum (EOF). Channel: " + i);
			} catch (NumberFormatException e) {
				throw new CorruptedFileException("Can't read physical maximum ("
						+ e.getMessage() + " not a number). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read digital minimum (EOF). Channel: " + i, e);
			} catch (NumberFormatException e) {
				throw new CorruptedFileException("Can't read digital minimum ("
						+ e.getMessage() + " not a number). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read digital maximum (EOF). Channel: " + i, e);
			} catch (NumberFormatException e) {
				throw new CorruptedFileException("Can't read digital maximum ("
						+ e.getMessage() + " not a number). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read prefiltering (EOF). Channel: " + i, e);
			}
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			...
			try {
				...
				throw new CorruptedFileException(
						"Can't read number of samples in each data record (EOF). Channel: "
								+ i, e);
			} catch (NumberFormatException e) {
				throw new CorruptedFileException(
						"Can't read number of samples in each data record ("
								+ e.getMessage() + " not a number). Channel: " + i, e);
			}

			// TODO - opravit na NSN, popr. zkontrolovat, jestli to takhle bude chodit
			// pro nesoudelny kombinace
		...
		try {
			header.setSamplingInterval(durationOfDRecord * 1000000
					/ globalNrOfSamplesInDR);
		} catch (ArithmeticException e) {
			throw new CorruptedFileException(
					"Can't calculate sampling interval (division by zero?).");
		}
		...
		for (int i = 0; i < numberOfSignals; i++) {
			try {
				...
				throw new CorruptedFileException(
						"Can't read reserved space (EOF). Channel: " + i, e);
			}
		}
	...
	private void loadDataRecords(BufferCreator loader) throws IOException {
		...
		for (int dataRecordsCounter = 0; dataRecordsCounter < numberOfDataRecords; dataRecordsCounter++) {
			...
			for (int signal = 0; signal < numberOfSignals; signal++) {
				...
				/*
				 * doplneni zbyvajicich framu data recordu: napr. pri
				 * globalNrOfSamplesInDR = 10 a nrOfSamplesInDRs = 3 se ve vyslednem
				 * poli predchozim kroku vyplni 3x 3 framy a 10. frame by zustal
				 * nevyplneny
				 */
	...
	private void headerChecksum() throws CorruptedFileException {
		if (this.bytesRead != headerSize) {
			...
			throw new CorruptedFileException(
					"Invalid header size (expected/loaded): " + headerSize + "/"
							+ this.bytesRead);
		} else {
	...
	private void dataChecksum() throws CorruptedFileException {
		if (loadedSamples == numberOfSamples) {
			...
			throw new CorruptedFileException(
					"Loaded samples not equal to expected (expected/loaded): "
							+ numberOfSamples + "/" + loadedSamples);
		}
		...
		if (incorrectReadings > 0) {
			throw new CorruptedFileException("Unexpected EOF (missing bytes: "
					+ incorrectReadings * 2 + ")");
		}

		if (nioStream.getRemaining() > 0) {
			throw new CorruptedFileException("Non-loaded remaining bytes: "
					+ nioStream.getRemaining());
		}
	...
	/**
	 * <p>
	 * Metoda na�te zadan� po�et znak� ze vstupn�ho streamu a ulo�� je do �et�zce.
	 * </p>
	 * 
	 * @param length
	 *          po�et znak� k na�ten�.
	 * @param stream
	 *          vstupn� stream.
	 * @return �et�zec s ulo�en�m vstupem.
	 * @throws EOFException
	 */
	private String getStringFromStream(int length, NioInputStream stream)
			throws EOFException {
		int tmp;
	...
	/**
	 * <p>
	 * Metoda na�te zadan� po�et znak� ze vstupn�ho streamu a p�evede je na cel�
	 * ��slo.
	 * </p>
	 * 
	 * @param length
	 ...
	 */
	private int getIntFromStream(int length, NioInputStream stream)
			throws EOFException, NumberFormatException {
		int tmp;
		String numstr = "";
	...
	/**
	 * <p>
	 * Metoda na�te zadan� po�et znak� ze vstupn�ho streamu a p�evede je na ��slo
	 * s plovouc� desetinnou ��rkou.
	 * </p>
	 * 
	 * @param length
	 *          po�et znak� k na�ten�.
	 ...
	 * @throws java.lang.NumberFormatException
	 */
	private float getFloatFromStream(int length, NioInputStream stream)
			throws EOFException, NumberFormatException {
		int tmp;
		String numstr = "";
	...
	private GregorianCalendar transformTimeAndDayToCalendar(String date,
			String time) throws CorruptedFileException {
		...
		if (timesArray.length != 3) {
			throw new CorruptedFileException("Incorrect time of acquisition: '"
					+ time + "'.");
		}

		if (Integer.valueOf(datesArray[2]) <= 70
				&& Integer.valueOf(datesArray[2]) >= 0) {
			year = new String("20" + datesArray[2]);
		} else {
			year = new String("19" + datesArray[2]);
		}

		gregorianCalendar.set(Integer.valueOf(year),
				Integer.valueOf(datesArray[1]), Integer.valueOf(datesArray[0]), Integer
						.valueOf(timesArray[0]), Integer.valueOf(timesArray[1]), Integer
						.valueOf(timesArray[2]));
		return gregorianCalendar;
	}
