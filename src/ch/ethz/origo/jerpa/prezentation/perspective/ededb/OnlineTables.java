...
public class OnlineTables extends JSplitPane implements ILanguage {
	...
	/**
	 * Method for filling experiment view table with information about
	 * experiments. Information depends on Owner/Subject button selection.
	 */
			...
			public void run() {
				if (session.isConnected()) {
					...
					if (availableExperiments != null) {
						JOptionPane.showMessageDialog(new JFrame(), availableExperiments.size() + " " + expInfoText,
								expInfoDesc, JOptionPane.INFORMATION_MESSAGE);
						...
						JOptionPane.showMessageDialog(new JFrame(), errorConnectionText, errorConnectionDesc,
	...
	public synchronized void updateDataTable() {
		...
		for (DataFileInfo info : dataFileInfos) {
			String downloadPath = controller.getDownloadPath() + File.separator + session.getUsername()
					+ File.separator + info.getExperimentId() + " - " + info.getScenarioName();
	...
	public void checkLocalCopies() {
		for (DataRowModel row : dataModel.getData()) {
			if (row.getState() != FileState.DOWNLOADING) {
				row.setState(controller.isAlreadyDownloaded(row.getFileInfo()));
			}
		}
	}
